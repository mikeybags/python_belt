...
class UserManager(models.Manager):
    def validate(self, first, last, email, password, confirm_pw, date_of_birth):
        errors = []
        if len(first) < 2:
        ...
        if date_of_birth >= datetime.date.today().isoformat():
    ...
    def register(self, first, last, email, password, date_of_birth):
        errors = []
        try:
            hashed = bcrypt.hashpw(password.encode(), bcrypt.gensalt())
            user = User.objects.create(first_name = first, last_name = last, email = email, pw_hash = hashed, date_of_birth = date_of_birth)
