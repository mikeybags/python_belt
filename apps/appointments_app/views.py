...
def index(request):
    ...
    appointments_today = Appointment.objects.filter(user = user_id).filter(date = today_iso)
    other_appointments = Appointment.objects.filter(user = user_id).exclude(date = today_iso)
    context = {'today': today,
